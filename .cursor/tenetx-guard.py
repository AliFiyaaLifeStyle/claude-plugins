#!/usr/bin/env python3
"""TenetX hook for Cursor Cloud Agents.

A Cursor Cloud Agent runs on a Cursor VM that never loads the laptop's
~/.cursor/hooks.json. It does load the cloned repo's .cursor/hooks.json, so
this file is committed as `.cursor/tenetx-guard.py` and wired there. It
...
The three separate names also work when TENETX_CURSOR_TOKEN is unset:
  TENETX_URL          control-plane origin (https://<org>.tenetx.ai)
  TENETX_ORG          org slug
  TENETX_VMCP_TOKEN   Cursor VMCP token

The environment's network access must allow the control-plane host (Custom,
plus the control-plane host). Missing credentials or a blocked download fail open so
cloud sessions are not bricked; every fail-open path writes a breadcrumb to
~/.tenetx/capture_failures.jsonl.

Keep byte-identical with cli-go/internal/hooks/cursor_cloud_hook.py;
tests/vmcp/test_cursor_cloud.py enforces that.
"""
...
# Stamped on the event so capture can file the run as Cursor Cloud rather than
# the IDE/CLI surface the guard detects on the VM. Read by
# tenetx/agent_observability/capture/adapters.py (_CURSOR_CLOUD_HOOK_SURFACE).
HOOK_SURFACE = "cursor-cloud"
BUNDLED_SECRET_NAME = "TENETX_CURSOR_TOKEN"
...
def _guard_path() -> str:
    override = _env("TENETX_CURSOR_CLOUD_GUARD")
    if override:
        return override
    home = _home()
    if home:
...
def _apply_bundled_secret() -> str | None:
    """Expand TENETX_CURSOR_TOKEN into URL, org, and the VMCP bearer.
...
def _tag_payload(payload: bytes) -> bytes:
    ...
    tags = (
        ...
        ("tenetx_remote_session_id", _env("CURSOR_AGENT_RUN_ID")),
    )
...
def main() -> int:
    ...
    if invalid:
        return _fail_open(
            invalid,
            detail=f"{BUNDLED_SECRET_NAME} is not a valid Cursor cloud token",
        )
    url = _env("TENETX_URL")
    org = _env("TENETX_ORG")
    token = _token()
        ...
        if _download_guard(url, org, token, guard) is None:
            if not cached:
                return _fail_open(
                    ...
                    path=guard,
                    detail=(
                        "could not download the Cursor guard from the "
